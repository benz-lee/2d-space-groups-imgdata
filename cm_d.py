...
for t in range(0,i,1):
    ...
    for _ in range(k):
        ...
        while True:
            ...
            y = random.randint(radius, int(h/4 - radius))
            valid = True
        ...
        for n in range(0, width, w):
            for m in range(0, height, h):
                ...
                dot1 = [(x - radius+n, h/2-y - radius+m), (x + radius+n, h/2-y + radius+m)]
                dot2 = [(w/2+x - radius+n, h/2+y - radius+m), (w/2+x + radius+n, h/2+y + radius+m)]
                dot3 = [(w/2+x - radius+n, h-y - radius+m), (w/2+x + radius+n, h-y + radius+m)]
