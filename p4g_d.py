...
for t in range(0,i,1):
    ...
    for n in range(0, width, 2*w):
        for m in range(0, height, 2*h):
            img.paste(basis, (n, m))
            img.paste(basis.transpose(Image.FLIP_TOP_BOTTOM),(n,m+h))#left-bottom
            img.paste(basis.transpose(Image.FLIP_LEFT_RIGHT), (n+w, m))#right-top
            img.paste(basis, (n+w, m+h))#right-bottom
