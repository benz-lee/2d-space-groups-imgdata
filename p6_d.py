    ...
    for _ in range(k):
        ...
        for n in range(0, width, 300):
            for m in range(0, height, h):
                ...
                dot3 = [(2*w-x - radius+n, h-y - radius+m), (2*w-x + radius+n, h-y + radius+m)]
                dot4 = [(2*w-x1 - radius+n, h-y1 - radius+m), (2*w-x1 + radius+n, h-y1 + radius+m)]
                dot5 = [(2*w-x2 - radius+n, h-y2 - radius+m), (2*w-x2 + radius+n, h-y2 + radius+m)] 
                ...
                dot9 = [(w-x - radius+n, h/2-y - radius+m), (w-x + radius+n, h/2-y + radius+m)]
                dot10 = [(w-x1 - radius+n, h/2-y1 - radius+m), (w-x1 + radius+n, h/2-y1 + radius+m)]
                dot11 = [(w-x2 - radius+n, h/2-y2 - radius+m), (w-x2 + radius+n, h/2-y2 + radius+m)]
