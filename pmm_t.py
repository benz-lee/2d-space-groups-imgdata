...
for t in range(0,i,1):
 ...
 while True:
    x1, y1 = random.randint(r, int(a/2-r)), random.randint(r, int(a/2-r))
    x2, y2 = random.randint(r, int(a/2-r)), random.randint(r, int(a/2-r))
    x3, y3 = random.randint(r, int(a/2-r)), random.randint(r, int(a/2-r))
    points = [
        (x1, y1),
        (x2, y2),
        (x3, y3)
 ...
 x8, y8 = x2, 100 - y2
 x9, y9 = x3, 100 - y3
 x10, y10 = 100 - x1, 100 - y1
 x11, y11 = 100 - x2, 100 - y2
 x12, y12 = 100 - x3, 100 - y3
 for n in range(0, width, 100):
    for m in range(0, height, 100):
        points = [
            (n+x1, m+y1),
            (n+x2, m+y2),
            (n+x3, m+y3)
